%PAPR_CCDF_QPSKforinputwithCP
clear all ; clc ; 
Modorder = 4 ; m = log2(Modorder) ; FFTCount = 2000 ;
N = 48*m*FFTCount ; databits = randi([0 1],1,N) ; pilotbitlen = 4*FFTCount ;
pilotbits = randi([0 1],1,pilotbitlen) ; pilotloc = [11 25 39 53] ;
[bpskout,qpskout] = QPSKDataBPSKPilot(pilotbits,databits) ;
datastart = 6 ;
...
for k = 1:48:length(qpskout)
    PGintemp = qpskout(1,k:k+47) ;
...
plot(paprvector,log10(cumulative),'g') ;
hold on ;
alphaA = 7.8 ;
