%BERTest16QAMtotalsystem
clear all ; clc ; 
FFTCount = 1500; Modorder = 16 ;
databits = randi([0 1],1,FFTCount*48*log2(Modorder)) ;
pilotbits = randi([0 1],1,FFTCount*4) ;
datastart = 6 ;
prefixlen = 16 ;
lengthEbNo = 10 ; incEbNo = 0.5 ;
[pilotmat,dataseq] = SixteenQAMDataBPSKPilot(pilotbits,databits) ;
...
demodout = SixteenQamDemod(demodin) ;
...
plot(EbNoaxis,errorcount,'-x') ; %+(log10(1/16^4))/100
hold on ; M = 16 ; linearSNR = 10.^(EbNoaxis./10); 
Pb1 = (1/4)*(erfc(sqrt(2*linearSNR/(5)))+erfc(3*sqrt(2*linearSNR/(5))));
Pb2 = (1/4)*(2*erfc(sqrt((2/5)*linearSNR)) + erfc(3*sqrt((2/5)*linearSNR)) - erfc(5*sqrt((2/5)*linearSNR))) ;
theoryBer = (1/2)*(Pb1 + Pb2) ;
plot(EbNoaxis,log10(theoryBer),'-o') ;