function y = SRRCFeyzi3(rolloff,uprate,filtlength)
    %this srrc designing filter is actually giving out the same coefficients as the built-in MATLAB function rcosdesign,
    ...
    filterpower = sum(abs(outmat).^2)./length(outmat) ;
    y = outmat.*filterpower ;
end